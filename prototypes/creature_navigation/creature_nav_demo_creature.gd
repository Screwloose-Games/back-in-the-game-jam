class_name CreatureNavDemoCreature
extends CharacterBody3D

## The ball the navigation module drives, and the label that says what it is doing.
##
## A CharacterBody3D RATHER THAN A KINEMATIC Node3D. The controllers emit a desired
## DIRECTION and SPEED, not a resolved position (see NavMotionCommand), so something has
## to resolve contact -- `move_and_slide` is that for free. A raw Node3D teleports into a
## wall whenever a steer is slightly off, and every such frame reads as a navigation bug
## rather than as the steering error it is.
##
...
##
## THE LABEL IS THE POINT OF THE PROTOTYPE. Four locomotion modes, a squeeze, a leap
## wind-up and an inspection are all things that look like "the ball moved oddly" without
## a word attached to them.
...
## Radii come from the navigation config so the ball is exactly the body being planned
## for. A prototype with its own radius demonstrates a creature nobody routed.
...
func _ready() -> void:
	...
	collision_layer = 0
	collision_mask = CreatureNavDemoMap.TERRAIN_LAYER
	_build_body()
	_build_label()
...
func _build_label() -> void:
	...
	# Readable from inside a tunnel, which is where half the interesting states happen.
...
## Turns one NavMotionCommand into movement. Everything navigation decides arrives here
## and nowhere else.
func _apply(command: NavMotionCommand, delta: float) -> void:
	...
	if navigation.local_planner.is_airborne():
		# Invariant 4: the velocity was fixed at launch and is re-emitted unchanged. The
		# body must not blend toward it, or the leap becomes steerable by accident.
...
##
## `ClearanceProfile.normal_body()` -- the shape every swept cast in the module is made
## with -- is a sphere of `normal_clearance()`, which is the body radius PLUS the safety
## margin. Give the collider only `normal_radius_equivalent` and the physics engine will
## happily park the body 1.10 m from a ceiling that navigation plans against at 1.25 m.
## From that pose `shape_sweep_clear`'s origin-overlap test reports penetration, EVERY
## candidate in every mode is rejected, and the creature never moves again -- observed, at
## (16, 9, 15) against the Dock ceiling, stationary with a COMPLETE route.
##
## The physical body must therefore be exactly what navigation plans with. The margin is
## not navigation's private bookkeeping; it is the reason the alien does not end up flush
## against geometry, and a body smaller than it is a body that can.
...
func _refresh_label() -> void:
	...
	var lines: Array[String] = [NavLocomotion.mode_name(planner.mode()).to_upper()]

	if navigation.command != null and navigation.command.recovering:
		# Above the inspection state on purpose: a body working itself off a wall or back
		# to one is the most misreadable thing in the scene, because it moves slowly in a

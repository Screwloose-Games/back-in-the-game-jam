...
var _tether_rope := CarryTetherRope.new()
...
func _ready() -> void:
	...
	# MovementKnobs.PLAYER_SPAWN before it enters the tree; the pose it arrives
	# with is the one R returns to.
	_spawn_transform = global_transform
	_head_camera.far = MovementKnobs.CAMERA_FAR
	_helmet_lamp.spot_range = MovementKnobs.HELMET_LAMP_RANGE
	_grab_ray.target_position = Vector3(0.0, 0.0, -MovementKnobs.GRAB_RANGE)
...
func _take_hold_of(object: RigidBody3D, grab_point: Vector3) -> void:
	...
	object.mass = MovementKnobs.CARRY_OBJECT_HELD_MASS

	var object_inverse := object.global_transform.affine_inverse()
	var half_span := global_transform.basis.x * MovementKnobs.GRIP_HAND_SEPARATION * 0.5
	_hand_object_anchors = [
	...
	_grip_carry_pose = Transform3D(
		_snap_basis_to_axes(_grip_start_pose.basis), MovementKnobs.GRIP_CARRY_OFFSET
	)
	_grip_settle = 0.0
	_grip_strain = 0.0
...
func _clip_tether_to(object: RigidBody3D, clip_point: Vector3) -> void:
	...
	_tether_suit_anchor = MovementKnobs.TETHER_ANCHOR_OFFSET
...
func _apply_grip_force(delta: float) -> void:
	...
	var angular_frequency := TAU * MovementKnobs.GRIP_SPRING_FREQUENCY
	var stiffness := reduced_mass * angular_frequency * angular_frequency
	var damping := 2.0 * MovementKnobs.GRIP_SPRING_DAMPING_RATIO * angular_frequency * reduced_mass
	var force_ceiling := MovementKnobs.GRIP_MAX_FORCE / float(hand_count)
	...
	if _grip_strain > MovementKnobs.GRIP_BREAK_DISTANCE:
	...
	for index: int in range(hand_points.size()):
		...
		_push_link_force(
			...
			-grip_force,
			suit_point,
			MovementKnobs.GRIP_SPIN_TRANSFER
		)
...
func _apply_grip_twist(delta: float, target_basis: Basis) -> void:
	if is_zero_approx(MovementKnobs.GRIP_TWIST_FREQUENCY) or _hand_object_anchors.size() < 2:
		return
	...
	var angular_frequency := TAU * MovementKnobs.GRIP_TWIST_FREQUENCY
	var stiffness := moment_of_inertia * angular_frequency * angular_frequency
	var damping := (
		2.0 * MovementKnobs.GRIP_TWIST_DAMPING_RATIO * angular_frequency * moment_of_inertia
	)
	var torque := clampf(
		twist_error * stiffness - twist_rate * damping,
		-MovementKnobs.GRIP_TWIST_MAX_TORQUE,
		MovementKnobs.GRIP_TWIST_MAX_TORQUE
	)

	var angular_impulse := twist_axis * torque * delta
	_held_object.apply_torque_impulse(angular_impulse)
	_add_spin_from_angular_impulse(-angular_impulse, MovementKnobs.GRIP_TWIST_SPIN_TRANSFER)
...
func _apply_tether_force(delta: float) -> void:
	...
	_tether_strain = maxf(_tether_distance - MovementKnobs.TETHER_LENGTH, 0.0)
	if _tether_strain > MovementKnobs.TETHER_BREAK_STRETCH:
		unclip_tether()
		return
	...
	tension = minf(tension, MovementKnobs.TETHER_MAX_FORCE)
	...
	_push_link_force(
		...
		suit_point,
		MovementKnobs.TETHER_SPIN_TRANSFER
	)
...
func _hold_anchor_clear(
	...
	var query := PhysicsRayQueryParameters3D.create(
		body_center, anchor, MovementKnobs.TETHER_ROPE_LAYERS
	)
	...
	return contact_point + contact_normal * MovementKnobs.TETHER_ROPE_RADIUS
...
func _push_link_force(
	...
	var suit_velocity_change := suit_force * delta / MovementKnobs.PLAYER_MASS
	velocity = (velocity + suit_velocity_change).limit_length(MovementKnobs.MAX_SPEED)
...
func _measure_reduced_mass(object: RigidBody3D) -> float:
	return 1.0 / (1.0 / MovementKnobs.PLAYER_MASS + 1.0 / object.mass)
...
func _update_orientation(delta: float) -> void:
	...
	var is_inertial := MovementKnobs.ROTATION_MODE == MovementKnobs.RotationMode.INERTIAL
	if is_inertial:
		var aim_gain := MovementKnobs.MOUSE_SENSITIVITY * MovementKnobs.ANGULAR_ACCELERATION
		angular_velocity.x += -mouse_motion.y * aim_gain
		angular_velocity.y += -mouse_motion.x * aim_gain
		angular_velocity.z += -roll_input * MovementKnobs.ROLL_RATE * delta
		angular_velocity = angular_velocity.limit_length(MovementKnobs.MAX_ANGULAR_SPEED)
	...
	var aim_roll := 0.0
	if not is_inertial:
		aim_pitch = -mouse_motion.y * MovementKnobs.MOUSE_SENSITIVITY
		aim_yaw = -mouse_motion.x * MovementKnobs.MOUSE_SENSITIVITY
		aim_roll = -roll_input * MovementKnobs.ROLL_RATE * delta
...
func _damp_angular_velocity(delta: float) -> void:
	...
	angular_velocity = angular_velocity.lerp(
		Vector3.ZERO, minf(MovementKnobs.ANGULAR_DRAG * delta, 1.0)
	)
	if stabilizers_engaged:
		angular_velocity = angular_velocity.lerp(
			Vector3.ZERO, minf(MovementKnobs.ANGULAR_STABILIZER_RATE * delta, 1.0)
		)
...
func _update_velocity(delta: float) -> void:
	...
	if stabilizers_engaged:
		_apply_braced_velocity_change(
			-velocity * minf(MovementKnobs.LINEAR_STABILIZER_RATE * delta, 1.0)
		)
...
func _apply_braced_velocity_change(unloaded_velocity_change: Vector3) -> void:
	...
	if _held_object != null:
		object_share = (
			MovementKnobs.GRIP_BRACING
			* _held_object.mass
			/ (MovementKnobs.PLAYER_MASS + _held_object.mass)
		)
	if not is_zero_approx(object_share):
		_held_object.apply_central_impulse(
			unloaded_velocity_change * MovementKnobs.PLAYER_MASS * object_share
		)
...
func _resolve_hull_contact(
	...
	if was_already_touching:
		velocity = velocity.lerp(Vector3.ZERO, minf(MovementKnobs.SCRAPE_FRICTION * delta, 1.0))
		return
	...
	velocity = (
		along_surface * (1.0 - MovementKnobs.COLLISION_FRICTION)
		- into_surface * MovementKnobs.COLLISION_RESTITUTION
	)
...
func _shove_loose_body(
	...
	var restitution := 0.0 if was_already_touching else MovementKnobs.COLLISION_RESTITUTION
	var reduced_mass := 1.0 / (1.0 / MovementKnobs.PLAYER_MASS + 1.0 / body.mass)
	var impulse_magnitude := -(1.0 + restitution) * closing_speed * reduced_mass

	velocity += contact_normal * (impulse_magnitude / MovementKnobs.PLAYER_MASS)
	body.apply_impulse(-contact_normal * impulse_magnitude, contact_point - body.global_position)

	var mass_ratio := body.mass / (body.mass + MovementKnobs.PLAYER_MASS)
...
func _apply_impact_spin(along_surface: Vector3, contact_point: Vector3) -> void:
	if is_zero_approx(MovementKnobs.COLLISION_SPIN_TRANSFER):
		return
	_add_spin_from_impulse(
		-along_surface * MovementKnobs.COLLISION_FRICTION,
		contact_point,
		MovementKnobs.COLLISION_SPIN_TRANSFER
	)
...
func _add_spin_from_impulse(
	...
	_add_spin_from_angular_impulse(
		lever_arm.cross(velocity_change * MovementKnobs.PLAYER_MASS), transfer
	)

class_name TestChamberGenerator
extends Node3D
...
func reset_carry_object() -> void:
	...
	_carry_object.global_transform = Transform3D(Basis.IDENTITY, MovementKnobs.CARRY_OBJECT_SPAWN)
...
func _assemble_combiner() -> void:
	...
	var wall_padding := Vector3.ONE * MovementKnobs.WALL_THICKNESS * 2.0
	combiner.add_child(
		_make_brush(
			Vector3.ZERO, MovementKnobs.CHAMBER_SIZE + wall_padding, CSGShape3D.OPERATION_UNION
		)
	)
	combiner.add_child(
		_make_brush(Vector3.ZERO, MovementKnobs.CHAMBER_SIZE, CSGShape3D.OPERATION_SUBTRACTION)
	)
...
func _add_divider(combiner: CSGCombiner3D) -> void:
	var divider_center := Vector3(0.0, 0.0, MovementKnobs.DIVIDER_DEPTH)
	combiner.add_child(
		_make_brush(
			divider_center,
			Vector3(
				MovementKnobs.CHAMBER_SIZE.x,
				MovementKnobs.CHAMBER_SIZE.y,
				MovementKnobs.DIVIDER_THICKNESS
			),
	...
	combiner.add_child(
		_make_brush(
			divider_center,
			Vector3(
				MovementKnobs.DIVIDER_GAP.x,
				MovementKnobs.DIVIDER_GAP.y,
				MovementKnobs.DIVIDER_THICKNESS * 3.0
			),
...
func _make_round_brush(brush_center: Vector3, brush_radius: float) -> CSGCylinder3D:
	...
	brush.height = MovementKnobs.CHAMBER_SIZE.y
	brush.sides = MovementKnobs.ROUND_PILLAR_SIDES
...
func _spawn_carry_object() -> void:
	var object_size := Vector3.ONE * MovementKnobs.CARRY_OBJECT_SIZE

	_carry_object = RigidBody3D.new()
	_carry_object.name = "CarryObject"
	_carry_object.mass = MovementKnobs.CARRY_OBJECT_MASS
	# Zero-g: nothing falls, it only goes where something pushed it.
	_carry_object.gravity_scale = 0.0
	_carry_object.linear_damp = MovementKnobs.CARRY_OBJECT_LINEAR_DAMP
	_carry_object.angular_damp = MovementKnobs.CARRY_OBJECT_ANGULAR_DAMP
	...
	_carry_object.position = MovementKnobs.CARRY_OBJECT_SPAWN
...
func _make_module_lamp() -> OmniLight3D:
	...
	lamp.light_color = MovementKnobs.CARRY_OBJECT_LIGHT_COLOR
	lamp.light_energy = MovementKnobs.CARRY_OBJECT_LIGHT_ENERGY
	lamp.omni_range = MovementKnobs.CARRY_OBJECT_LIGHT_RANGE
	lamp.omni_attenuation = MovementKnobs.CARRY_OBJECT_LIGHT_ATTENUATION
	lamp.shadow_enabled = MovementKnobs.CARRY_OBJECT_LIGHT_SHADOWS
...
func _make_glowing_material() -> StandardMaterial3D:
	...
	material.emission = MovementKnobs.CARRY_OBJECT_LIGHT_COLOR
	material.emission_energy_multiplier = MovementKnobs.CARRY_OBJECT_GLOW
	return material

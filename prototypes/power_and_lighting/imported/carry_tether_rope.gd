...
func reset(object_anchor: Vector3, suit_anchor: Vector3) -> void:
	var segment_count := maxi(
		1, int(roundf(MovementKnobs.TETHER_LENGTH / MovementKnobs.TETHER_ROPE_SEGMENT))
	)
	var point_count := segment_count + 1
	_positions.resize(point_count)
	_previous_positions.resize(point_count)
	_segment_rest_length = MovementKnobs.TETHER_LENGTH / float(segment_count)
...
func step(
	...
	for iteration: int in range(MovementKnobs.TETHER_ROPE_ITERATIONS):
...
func _carry_momentum(delta: float) -> void:
	var retention := clampf(1.0 - MovementKnobs.TETHER_ROPE_DAMPING * delta, 0.0, 1.0)
...
func _pull_segment_together(index: int, last_index: int) -> void:
	...
	if length > _segment_rest_length:
		strength = 1.0
	elif length < _segment_rest_length * MovementKnobs.TETHER_ROPE_SPREAD:
		target = _segment_rest_length * MovementKnobs.TETHER_ROPE_SPREAD
	else:
		return
...
func _lift_point_clear(
	...
	var query := PhysicsRayQueryParameters3D.create(
		clear_position, _positions[index], MovementKnobs.TETHER_ROPE_LAYERS
	)
	...
	var lifted := contact_point + contact_normal * MovementKnobs.TETHER_ROPE_RADIUS
	...
	_previous_positions[index] = (
		lifted - point_velocity.slide(contact_normal) * (1.0 - MovementKnobs.TETHER_ROPE_FRICTION)
	)

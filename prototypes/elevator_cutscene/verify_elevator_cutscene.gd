...
const ANIMATED_NODES := {
	...
	"HUD/HelmetHud": "Control",
	"HUD/HelmetHud/BeginWorkday": "Label",
...
func _verify_scene_paths() -> void:
	...
	for path: String in ANIMATED_NODES:
		var node := prototype.get_node_or_null(NodePath(path))
		if node == null:
	...
	# other two instead.
	var miner_a: MinerRig = prototype.get_node("Miners/MinerA")
	var miner_b: MinerRig = prototype.get_node("Miners/MinerB")
	var miner_c: MinerRig = prototype.get_node("Miners/MinerC")
	...
	_check(
		...
		(
			...
			and is_equal_approx(miner_a.visor_energy, ElevatorCutsceneKnobs.VISOR_IDLE_ENERGY)
			and is_equal_approx(miner_c.visor_energy, ElevatorCutsceneKnobs.VISOR_IDLE_ENERGY)
		),
	...
	var door_left: Node3D = prototype.get_node("ElevatorCar/Doors/DoorLeft")
	...
	_check(
		...
		(
			is_equal_approx(door_left.position.x, -ElevatorCutsceneKnobs.DOOR_CLOSED_X)
			and is_equal_approx(leaf.get_aabb().size.x, ElevatorCutsceneKnobs.DOOR_TRAVEL)
			and is_equal_approx(leaf.get_aabb().end.x, -ElevatorCutsceneKnobs.DOOR_CLOSED_X)
		),
		(
			...
			% [
				...
				leaf.get_aabb().end.x,
				ElevatorCutsceneKnobs.DOOR_TRAVEL,
				ElevatorCutsceneKnobs.DOOR_CLOSED_X
			]
		)
	)
	...
	# Two HUDs, and prefab_hud_04's minimap SubViewport carries a Camera3D that
	# would fail [camera] with a message about cameras rather than about the HUD.
	var hud_binding: Node = prototype.get_node("Player/PlayerBody/UI/HudBinding")
	_check(
		"[hud] the prototype's overlay is the only one",
		hud_binding.hud_scene == null and hud_binding.hud() == null,
		"prefab_player instanced its own HUD over the cutscene's"
	)

	# The pause menu is the one path that can hand input back mid-shot.
	_check(
		"[pause] no menu can un-park the rig",
		prototype.get_node_or_null("Player/PlayerBody/UI/PauseScreen") == null,
		"PauseScreen is still in the tree; unpausing re-enables input mid-cutscene"
	)
...
func _verify_framing(prototype: Node3D) -> void:
	var screen: ElevatorScreen = prototype.get_node("ElevatorCar/QuotaScreen")
	...
	var position := ElevatorCutsceneKnobs.CLOSEUP_POSITION
	var target := ElevatorCutsceneKnobs.CLOSEUP_TARGET
	...
	_check(
		...
		(
			...
			and _near(distance, ElevatorCutsceneKnobs.CLOSEUP_DISTANCE)
		),
		(
			...
			% [
				...
				position,
				target,
				ElevatorCutsceneKnobs.CLOSEUP_DISTANCE
			]
		)
	)
	...
	var half_height := distance * tan(deg_to_rad(ElevatorCutsceneKnobs.CUTSCENE_FOV) * 0.5)
...
func _cover_distance(glass: Vector2) -> float:
	var half := tan(deg_to_rad(ElevatorCutsceneKnobs.CUTSCENE_FOV * 0.5))
...
func _verify_camera_seam() -> void:
	...
	var rig: CutscenePlayerRig = prototype.get_node("PlayerRig")
...
func _verify_animation() -> void:
	...
	_check(
		"[root_node] resolves to the level root",
		animation_player.get_node_or_null(animation_player.root_node) == prototype,
		(
	...
	_check(
		...
		is_equal_approx(animation.length, ElevatorCutsceneKnobs.CUTSCENE_LENGTH),
		"clip is %.2fs, knobs say %.2fs" % [animation.length, ElevatorCutsceneKnobs.CUTSCENE_LENGTH]
	)
	...
	for track in animation.get_track_count():
		...
		if prototype.get_node_or_null(node_path) == null:
		...
		(
			...
			and _near(
				animation.value_track_interpolate(
					shot_track, ElevatorCutsceneKnobs.PROLOGUE_LENGTH
				),
				0.0
			)
			and _near(
				animation.value_track_interpolate(
					shot_track, ElevatorCutsceneKnobs.CUTSCENE_LENGTH
				),
				0.0
			)
		),
	...
	if anchor_track >= 0:
		...
		reveals_at = animation.position_track_interpolate(
			anchor_track, ElevatorCutsceneKnobs.PROLOGUE_END
		)
	_check(
		"[prologue] the camera opens on the close-up and reveals by PROLOGUE_END",
		(
			anchor_track >= 0
			and opens_on.distance_to(ElevatorCutsceneKnobs.CLOSEUP_POSITION) < 0.001
			and reveals_at.distance_to(ElevatorCutsceneKnobs.SHOT1_POSITION) < 0.001
		),
...
func _verify_window_dispatch() -> void:
	var prototype: Node3D = await _spawn()
	var cutscene: ElevatorCutscenePlayer = prototype.get_node("Cutscenes/ElevatorIntro")
	var effects: ElevatorEffects = prototype.get_node("Cutscenes/ElevatorIntro/Effects")
	...
	cutscene.advance(ElevatorCutsceneKnobs.CUTSCENE_LENGTH + 1.0)
	_check(
		"[window] all five fired in one tick",
		cutscene.get_fired_count() == 5,
		"%d of 5 effects fired from a single oversized delta" % cutscene.get_fired_count()
	)
	_check(
		"[window] end state reached",
		effects.workday_begun and effects.doors_unlocked and effects.hud_online,
		(
			"flags after a full run: workday=%s doors=%s hud=%s"
			% [effects.workday_begun, effects.doors_unlocked, effects.hud_online]
		)
	)
...
func _verify_skip_equivalence() -> void:
	...
	# leave something covering the screen. The other three are the old times shifted.
	for skip_at: float in [0.0, 1.5, 4.0, 13.5, 23.5]:
...
func _snapshot(prototype: Node3D, label: String) -> Dictionary:
	...
	var monitor: ElevatorMonitorShot = prototype.get_node("HUD/MonitorShot")
	_check(
	...
	return {
		...
		"door_left_x": prototype.get_node("ElevatorCar/Doors/DoorLeft").position.x,
		"door_right_x": prototype.get_node("ElevatorCar/Doors/DoorRight").position.x,
		"doors_solid": car.are_doors_solid(),
		"hud_alpha": prototype.get_node("HUD/HelmetHud").modulate.a,
		"title_alpha": prototype.get_node("HUD/HelmetHud/BeginWorkday").modulate.a,
		"visor_energy": (prototype.get_node("Miners/MinerB") as MinerRig).visor_energy,
		"gesture_blend": (prototype.get_node("Miners/MinerB") as MinerRig).get_gesture_blend(),
		"strobe_energy": prototype.get_node("ElevatorCar/WarningStrobe").light_energy,

class_name ElevatorCutscenePlayer
extends Node3D
...
## prototype exists to test, and it earns its keep three times over - on skip, on
## scrub, and (later) on the network.
...
var _effects_target: ElevatorEffects

@onready var _animation_player: AnimationPlayer = $AnimationPlayer
@onready var _camera_anchor: Node3D = $CameraAnchor
@onready var _preview_camera: Camera3D = $CameraAnchor/PreviewCamera
...
func bind(
	...
	effects_target: ElevatorEffects
) -> void:
...
func enter(blend_time: float) -> void:
	if _phase != Phase.IDLE:
		...
		# is at least legible.
		push_warning("elevator_intro requested while already playing; dropped.")
		return
	if data == null:
		push_error("ElevatorCutscenePlayer has no CutsceneData; nothing to play.")
		return
...
func _cleanup(blend_time: float) -> void:
	...
	_hud.set_helmet_online(_effects_target.hud_online)
...
func _exit_blend_time() -> float:
	return ElevatorCutsceneKnobs.EXIT_BLEND_TIME
...
func _warn_if_exit_is_inside_geometry() -> void:
	...
	shape.radius = ElevatorCutsceneKnobs.RIG_HULL_RADIUS
...
func _load_data() -> void:
	...
	for failure: String in data.validate(_length):
		push_warning("elevator_intro.cutscene.tres: %s" % failure)

...
const ANIM_PATH := "res://prototypes/elevator_cutscene/animations/elevator_intro.anim.tres"
const RESET_PATH := "res://prototypes/elevator_cutscene/animations/elevator_intro_reset.anim.tres"
const LIBRARY_PATH := "res://prototypes/elevator_cutscene/animations/elevator_intro_library.tres"

const GESTURE_LIBRARY_PATH := "res://prototypes/elevator_cutscene/animations/miner_gesture_library.tres"
const GESTURE_CLIP_PATH := "res://prototypes/elevator_cutscene/animations/miner_helmet_gesture.anim.tres"
...
const HELMET_HUD := "HUD/HelmetHud:modulate"
const TITLE := "HUD/HelmetHud/BeginWorkday:modulate"
...
func _build_gesture_library() -> void:
	...
	var target := _gesture_target(character, skeleton, idle, upper)
	var raised := _solve_two_bone(skeleton, idle, upper, lower, tip, target)
	...
	anim.length = ElevatorCutsceneKnobs.ANIMATION_STEP
	anim.loop_mode = Animation.LOOP_NONE
	anim.step = ElevatorCutsceneKnobs.ANIMATION_STEP
...
## below it.
func _gesture_target(character: Node, skeleton: Skeleton3D, idle: Animation, upper: int) -> Vector3:
	...
	var offset := ElevatorCutsceneKnobs.GESTURE_HAND_OFFSET
...
func _solve_two_bone(
	skeleton: Skeleton3D, idle: Animation, upper: int, lower: int, tip: int, target: Vector3
) -> Array:
	...
	var pole := ElevatorCutsceneKnobs.GESTURE_ELBOW_POLE
	pole = Vector3(signf(shoulder.x) * pole.x, pole.y, pole.z)
...
func _build_intro() -> Animation:
	...
	anim.length = ElevatorCutsceneKnobs.CUTSCENE_LENGTH
	anim.loop_mode = Animation.LOOP_NONE
	anim.step = ElevatorCutsceneKnobs.ANIMATION_STEP
...
func _build_camera(anim: Animation) -> void:
	...
	var step := ElevatorCutsceneKnobs.ANIMATION_STEP
	var prologue := ElevatorCutsceneKnobs.PROLOGUE_LENGTH
	var reveal := ElevatorCutsceneKnobs.PROLOGUE_END
	var cut := ElevatorCutsceneKnobs.SHOT2_CUT_TIME
	var dolly_end := cut + ElevatorCutsceneKnobs.SHOT2_PUSH_IN_DURATION
	var match_cut := ElevatorCutsceneKnobs.MATCH_CUT_TIME
	...
	_key_pose(
		...
		rotation_track,
		0.0,
		ElevatorCutsceneKnobs.CLOSEUP_POSITION,
		ElevatorCutsceneKnobs.CLOSEUP_TARGET
	)
	_key_pose(
		anim,
		position_track,
	...
	for index in DOLLY_STEPS + 1:
		...
		_key_pose(
			...
			ElevatorCutsceneKnobs.CLOSEUP_POSITION.lerp(
				ElevatorCutsceneKnobs.SHOT1_POSITION, eased
			),
			ElevatorCutsceneKnobs.CLOSEUP_TARGET.lerp(ElevatorCutsceneKnobs.SHOT1_TARGET, eased)
		)
	...
	_key_pose(
		...
		rotation_track,
		reveal,
		ElevatorCutsceneKnobs.SHOT1_POSITION,
		ElevatorCutsceneKnobs.SHOT1_TARGET
	)
	_key_pose(
		anim,
		position_track,
	...
	for index in DOLLY_STEPS + 1:
		...
		_key_pose(
			...
			ElevatorCutsceneKnobs.SHOT2_START_POSITION.lerp(
				ElevatorCutsceneKnobs.SHOT2_END_POSITION, eased
			),
			ElevatorCutsceneKnobs.SHOT2_START_TARGET.lerp(
				ElevatorCutsceneKnobs.SHOT2_END_TARGET, eased
			)
		)
	...
	_key_pose(
		...
		rotation_track,
		match_cut - step,
		ElevatorCutsceneKnobs.SHOT2_END_POSITION,
		ElevatorCutsceneKnobs.SHOT2_END_TARGET
	)
	_key_pose(
		anim,
		position_track,
		rotation_track,
		match_cut,
		ElevatorCutsceneKnobs.EYE_POSITION,
		ElevatorCutsceneKnobs.EYE_TARGET
	)
	_key_pose(
		anim,
		position_track,
		rotation_track,
		ElevatorCutsceneKnobs.CUTSCENE_LENGTH,
		ElevatorCutsceneKnobs.EYE_POSITION,
		ElevatorCutsceneKnobs.EYE_TARGET
	)
...
func _build_monitor_shot(anim: Animation) -> void:
	...
	var cut := ElevatorCutsceneKnobs.PROLOGUE_LENGTH
	anim.track_insert_key(shot, 0.0, 1.0)
	anim.track_insert_key(shot, cut - ElevatorCutsceneKnobs.ANIMATION_STEP, 1.0)
	anim.track_insert_key(shot, cut, 0.0)
	anim.track_insert_key(shot, ElevatorCutsceneKnobs.CUTSCENE_LENGTH, 0.0)


func _build_doors(anim: Animation) -> void:
	var closed := ElevatorCutsceneKnobs.DOOR_CLOSED_X
	var open := closed + ElevatorCutsceneKnobs.DOOR_TRAVEL
	var start := ElevatorCutsceneKnobs.DOORS_OPEN_TIME
	var end := start + ElevatorCutsceneKnobs.DOOR_TRAVEL_DURATION
	...
	for side: float in [-1.0, 1.0]:
		...
		anim.position_track_insert_key(track, start, Vector3(side * closed, 0.0, 0.0))
		# Eased so the leaves start and stop like something heavy rather than
		...
		for index in DOLLY_STEPS + 1:
			...
			anim.position_track_insert_key(
				track,
				lerpf(start, end, fraction),
				Vector3(side * lerpf(closed, open, eased), 0.0, 0.0)
			)
...
func _build_gesture(anim: Animation) -> void:
	...
	var start := ElevatorCutsceneKnobs.GESTURE_START
	var duration := ElevatorCutsceneKnobs.GESTURE_DURATION
	...
	anim.track_insert_key(blend, ElevatorCutsceneKnobs.CUTSCENE_LENGTH, 0.0)

	var idle := ElevatorCutsceneKnobs.VISOR_IDLE_ENERGY
	anim.track_insert_key(visor, 0.0, idle)
	anim.track_insert_key(visor, reach, idle)
	anim.track_insert_key(visor, reach + 0.25, ElevatorCutsceneKnobs.VISOR_LIT_ENERGY)
	anim.track_insert_key(
		visor, ElevatorCutsceneKnobs.CUTSCENE_LENGTH, ElevatorCutsceneKnobs.VISOR_LIT_ENERGY
	)
...
## The helmet overlay fading up on the match cut, and the title card.
##
## ALPHA, NOT `visible`. A bool track would be seek-safe too, but then two
## properties would answer "is the HUD showing" and only one of them would be
## right after a skip. One property, one track, one authority.
func _build_hud(anim: Animation) -> void:
	var hud := _add_value_track(anim, HELMET_HUD)
	var title := _add_value_track(anim, TITLE)

	var cut := ElevatorCutsceneKnobs.MATCH_CUT_TIME
	var faded := cut + ElevatorCutsceneKnobs.HUD_FADE_IN
	var clear := Color(1.0, 1.0, 1.0, 0.0)
	var solid := Color(1.0, 1.0, 1.0, 1.0)

	anim.track_insert_key(hud, 0.0, clear)
	anim.track_insert_key(hud, cut - ElevatorCutsceneKnobs.ANIMATION_STEP, clear)
	anim.track_insert_key(hud, faded, solid)
	anim.track_insert_key(hud, ElevatorCutsceneKnobs.CUTSCENE_LENGTH, solid)
	...
	var hold := ElevatorCutsceneKnobs.TITLE_HOLD
	var clunk := ElevatorCutsceneKnobs.CLUNK_TIME
	...
	anim.track_insert_key(title, clunk + hold * 0.25, solid)
	anim.track_insert_key(title, clunk + hold * 0.5, clear)
	anim.track_insert_key(title, ElevatorCutsceneKnobs.CUTSCENE_LENGTH, clear)
...
func _build_car(anim: Animation) -> void:
	...
	var clunk := ElevatorCutsceneKnobs.CLUNK_TIME
	var peak := ElevatorCutsceneKnobs.STROBE_PEAK_ENERGY
	...
	anim.track_insert_key(strobe, ElevatorCutsceneKnobs.CUTSCENE_LENGTH, 0.0)
	...
	var lit := 1.0
	var dim := ElevatorCutsceneKnobs.SCREEN_DROPOUT_LEVEL
	anim.track_insert_key(screen, 0.0, lit)
	for dropout_time: float in ElevatorCutsceneKnobs.SCREEN_DROPOUT_TIMES:
		anim.track_insert_key(screen, dropout_time, dim)
		anim.track_insert_key(
			screen, dropout_time + ElevatorCutsceneKnobs.SCREEN_DROPOUT_DURATION, lit
		)
	anim.track_insert_key(screen, ElevatorCutsceneKnobs.CUTSCENE_LENGTH, lit)
...
func _build_reset() -> Animation:
	...
	anim.position_track_insert_key(anchor_position, 0.0, ElevatorCutsceneKnobs.SHOT1_POSITION)
	var anchor_rotation := _add_transform_track(anim, Animation.TYPE_ROTATION_3D, ANCHOR)
	anim.rotation_track_insert_key(
		anchor_rotation,
		0.0,
		_look_quaternion(ElevatorCutsceneKnobs.SHOT1_POSITION, ElevatorCutsceneKnobs.SHOT1_TARGET)
	)
	...
	anim.track_insert_key(
		_add_value_track(anim, VISOR), 0.0, ElevatorCutsceneKnobs.VISOR_IDLE_ENERGY
	)
	anim.track_insert_key(_add_value_track(anim, HELMET_HUD), 0.0, Color(1.0, 1.0, 1.0, 0.0))

class_name CoreLoopSuit
extends CharacterBody3D

## Six-degrees-of-freedom suit-thruster controller that can take hold of a heavy
## module and haul it around, drill, and sprint.
##
## COPIED FROM object_carrying/carrier_player.gd. That file is where the grip,
## the tether and the braced carry are explained at length, and it is still the
## place to read about why any of them work the way they do - none of that is
## repeated here, because two copies of an explanation is one copy that goes
## stale. Grip and tether values still come from carry_knobs.gd, read directly.
##
## THREE DEPARTURES FROM THE ORIGINAL:
##
## 1. NO SETTINGS RESOURCE. Retyping the original's CarrySettings would put six
##    grip-and-tether sliders on a panel with twelve rows of room and a monster
##    to tune. What is genuinely open here arrives through `max_speed`,
##    `thrust_acceleration` and set_camera_far() instead.
## 2. SPRINT IS BACK, lifted from navigation/zero_g_player.gd unchanged in
##    behaviour. This map is 200 m across and has something in it that moves at
##    9 m/s, so outrunning it has to be a verb the suit has.
## 3. THE SPEED CAP IS `_current_speed_cap`, NOT `max_speed`. Both the thrust
##    clamp and the braced-carry clamp read the eased ceiling, so hauling the
##    cube while sprinting is not silently capped at the cruising speed.
...
## True while sprint is held. Read by the debug HUD and by the noise bus, which
## charges sprinting a louder radius than cruising.
var sprint_engaged := false

## Cruising speed ceiling, metres per second. Pushed in from the prototype root,
## which owns the slider.
var max_speed := CoreLoopKnobs.MAX_SPEED
## Thrust along the body's own axes, metres per second squared.
var thrust_acceleration := CoreLoopKnobs.THRUST_ACCELERATION
...
## The ceiling velocity is actually clamped to. Rises the instant sprint goes
## down and eases back afterwards, so releasing coasts rather than snapping.
var _current_speed_cap := CoreLoopKnobs.MAX_SPEED
...
func _ready() -> void:
	...
	_head_camera.far = CoreLoopKnobs.CAMERA_FAR
	_helmet_lamp.spot_range = CoreLoopKnobs.HELMET_LAMP_RANGE
...
## Where the drill beam and anything else that aims down the crosshair attaches.
func get_head_camera() -> Camera3D:
	return _head_camera


## The suit's lamp, for whatever is dimming it with the battery.
func get_helmet_lamp() -> SpotLight3D:
	return _helmet_lamp


## The camera's clip plane. Belongs to the scene rather than to the suit: how far
## you can see is a property of the murk.
...
func _take_hold_of(object: RigidBody3D, grab_point: Vector3) -> void:
	...
	object.mass = CoreLoopKnobs.CUBE_HELD_MASS
...
func _clip_tether_to(object: RigidBody3D, clip_point: Vector3) -> void:
	...
	_tether_rope.reset(clip_point, global_transform * _tether_suit_anchor, CarryKnobs.TETHER_LENGTH)
	_tether_distance = 0.0
	_tether_strain = 0.0
	_tether_drape = 0.0
...
func _push_link_force(
	...
	velocity = (velocity + suit_velocity_change).limit_length(_current_speed_cap)
...
func _update_velocity(delta: float) -> void:
	...
	_apply_braced_velocity_change(
		global_transform.basis * thrust_input * _thrust_this_frame() * delta
	)
...
## Thrust for this frame. Sprint multiplies the PUSHED acceleration rather than
## the const, so the multiplier goes on meaning "this much faster than normal"
## whatever normal has been set to.
func _thrust_this_frame() -> float:
	if sprint_engaged:
		return thrust_acceleration * CoreLoopKnobs.SPRINT_ACCELERATION_MULTIPLIER
	return thrust_acceleration


## Tracks the ceiling the speed is clamped to: raised the instant sprint goes
## down, eased back afterwards so releasing coasts rather than snapping. Easing
## the clamp rather than the speed is what keeps thrust from outrunning the
## ceiling on the way down.
func _update_speed_cap(delta: float) -> void:
	var target_speed_cap := max_speed
	if sprint_engaged:
		target_speed_cap *= CoreLoopKnobs.SPRINT_SPEED_MULTIPLIER

	if target_speed_cap >= _current_speed_cap:
		_current_speed_cap = target_speed_cap
	else:
		_current_speed_cap = move_toward(
			_current_speed_cap, target_speed_cap, CoreLoopKnobs.SPRINT_FALLOFF_RATE * delta
		)
...
## so both bodies set off, or pull up, together. Unbraced, the module hears about
## the burst only through hands out in front of your centre of mass, and the
## lever arm pitches you over every time. carrier_player.gd works through why at
## length, including why it costs no speed.

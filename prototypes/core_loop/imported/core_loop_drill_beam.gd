...
## that is already stale. There is also nothing to broad-phase: there are eight
## nodes.
##
## COPIED FROM drill_and_mining/drill_beam.gd. ONE THING IS ADDED: the drill costs
## power. That prototype's README says outright that the drill costs nothing to
## run, and finding out what it should cost is most of why this prototype exists,
## so the draw is here and its rate is a slider.
##
## THE POWER GATE IS ON `_is_firing`, NOT INSIDE `_fire`. Everything downstream
## reads that one flag - the beam is stowed, no rock is cut, and the noise bus
## reports silence - so a flat battery makes the drill go quiet as well as
## harmless. Gating the carve alone would leave a lit beam that removes nothing
## and still calls the creature.
...
## The battery the drill runs off: the suit's, assigned by the prototype root.
## Left null, the drill is free and behaves exactly as the donor prototype's does.
var power_store: PowerStore

## Whether the trigger only counts while the mouse is captured.
##
## ON in play, and it has to be: ESC frees the mouse to reach the tuning panel,
## and without this a click on a slider also cuts a hole in whatever you were
## looking at.
##
## A HEADLESS RUN CANNOT SATISFY IT. There is no window to capture, so
## Input.mouse_mode never leaves MOUSE_MODE_VISIBLE however it is assigned, and
## the drill can never fire - which a verifier reads as "the beam is broken"
## rather than "the guard is doing its job". verify_core_loop.gd turns this off,
## and that is the only thing that ever should.
var mouse_capture_required := true
...
var _carve_rate := CoreLoopKnobs.CARVE_RATE
var _carve_radius := CoreLoopKnobs.CARVE_RADIUS
var _range := CoreLoopKnobs.DRILL_RANGE
var _debris_rate := CoreLoopKnobs.DEBRIS_SPAWN_RATE
var _power_per_second := CoreLoopKnobs.DRILL_POWER_PER_SECOND
...
func _physics_process(delta: float) -> void:
	...
	var panel_is_closed := (
		not mouse_capture_required or Input.mouse_mode == Input.MOUSE_MODE_CAPTURED
	)
	_is_firing = (
		Input.is_action_pressed(CoreLoopKnobs.DRILL_ACTION) and panel_is_closed and has_power()
	)
	if not _is_firing:
		_stow()
		return
...
func apply_tuning(
	...
	_power_per_second = power_per_second


## Whether there is anything left to run on. True with no store bound, so the
## drill still works in a scene that has no power in it at all.
func has_power() -> bool:
	if not CoreLoopKnobs.DRILL_DRAWS_POWER or power_store == null:
		return true
	return not power_store.is_empty()


## Takes this frame's charge. spend() returns what it could actually give, which
## is ignored here on purpose: running the battery to exactly zero is the last
## frame the drill fires, and has_power() stops the next one.
func _spend_power(delta: float) -> void:
	if not CoreLoopKnobs.DRILL_DRAWS_POWER or power_store == null:
		return
	power_store.spend(_power_per_second * delta)
...
func _throw_debris(hit_point: Vector3, aim: Vector3, surface_normal: Vector3, delta: float) -> void:
	if not CoreLoopKnobs.DEBRIS_ENABLED or debris_pool == null or _debris_rate <= 0.0:
		return
...
func _heft_of(wait: float) -> float:
	return clampf(wait * _debris_rate / CoreLoopKnobs.DEBRIS_HEFT_WAITS, 0.0, 1.0)
...
func _show_beam(hit_point: Vector3, has_hit: bool) -> void:
	var muzzle := CoreLoopKnobs.MUZZLE_OFFSET
	...
	var radius := (
		CoreLoopKnobs.BEAM_RADIUS * (1.0 + randf_range(-1.0, 1.0) * CoreLoopKnobs.BEAM_JITTER)
	)
...
func _build_impact() -> void:
	...
	spark.radius = CoreLoopKnobs.IMPACT_DOT_RADIUS
	spark.height = CoreLoopKnobs.IMPACT_DOT_RADIUS * 2.0

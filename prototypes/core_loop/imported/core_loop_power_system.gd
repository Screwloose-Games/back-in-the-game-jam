class_name CoreLoopPowerSystem
extends Node

## The rules about where power goes. The two batteries themselves know none of
## this - see power_store.gd.
##
## Two ways charge MOVES, and they run in opposite directions:
##
##   TETHER  cube -> suit, automatically, whenever the line is clipped on.
##   CRANK   nothing -> cube, while the crank is engaged.
##
## Cranking is told to this node rather than read off the keyboard by it, so that
## what counts as cranking stays the prototype root's business.
##
## COPIED FROM power_and_lighting/power_system.gd, and the copy exists for one
## reason: bind() is statically typed to that prototype's CarrierSuit.
##
## THE TWO WAYS CHARGE IS SPENT ARE BOTH ELSEWHERE, and neither belongs here. The
## lamp is the suit store's own idle drain, and the drill spends from that store
## directly - see CoreLoopDrillBeam. Both happen whatever this node thinks,
## including while the suit is charging, so the tether has to out-pay them rather
## than pause them. That is what makes SUIT_CHARGE_PER_SECOND against
## SUIT_DRAIN_PER_SECOND plus DRILL_POWER_PER_SECOND a real ratio instead of three
## independent numbers.
##
...
## Charge per second the suit pulls from a tethered cube. Seeded from
## CoreLoopKnobs and moved by the tuning panel.
var charge_per_second := CoreLoopKnobs.SUIT_CHARGE_PER_SECOND

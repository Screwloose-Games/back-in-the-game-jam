...
@export_range(2, 200, 1) var number_of_points: int = 10:
	set(value):
		number_of_points = value
		call_deferred("_regenerate")

@export_range(0.01, 2.0, 0.01) var point_size: float = 0.1:
	set(value):
		point_size = value
...
func _create_visualizer():
	...
	material.albedo_color = Color(0.0, 0.3, 1.0, 0.8)
...
func _regenerate():
	...
	call_deferred("_generate_points")


func _generate_points():
	var mm := point_mesh.multimesh
	var count := number_of_points * number_of_points * number_of_points
...
# this needs to be refactored
	for z in range(number_of_points):
		var zpos := lerpf(-half.z, half.z, float(z) / float(number_of_points - 1))
		for y in range(number_of_points):
			var ypos := lerpf(-half.y, half.y, float(y) / float(number_of_points - 1))
			for x in range(number_of_points):
				var xpos := lerpf(-half.x, half.x, float(x) / float(number_of_points - 1))
				var t := Transform3D()
				t.origin = Vector3(xpos, ypos, zpos)
				t.basis = Basis.IDENTITY.scaled(Vector3.ONE * point_size)

...
##
## Every tunable value lives in drill_movement_knobs.gd. Unlike the navigation
## prototype's original, this copy holds no settings resource: movement is
## settled and is not what this prototype is asking about, so the consts are read
## straight. The two values that do belong to a scene rather than to the suit -
## the clip plane and the lamp's throw - are pushed in by the prototype root
...
## Seed only - _update_speed_cap owns this from the first physics frame.
var _current_speed_cap := DrillMovementKnobs.MAX_SPEED
...
func _ready() -> void:
	...
	_head_camera.far = DrillMovementKnobs.CAMERA_FAR
	_helmet_lamp.spot_range = DrillMovementKnobs.HELMET_LAMP_RANGE
	# Both are overwritten by the prototype root from ../drill_knobs.gd on the
	# same frame; these are only what a bare suit would come up as.
	capture_mouse()
...
## Hard clip distance, in metres. The scene owns this rather than the suit
## because it has to agree with the fog the scene is drawing.
func set_camera_far(distance: float) -> void:
	_head_camera.far = distance


## The helmet lamp's throw in metres, its cone half-angle in degrees, and its
## brightness. The scene owns all three: in this prototype the lamp is not a
## mechanic, it is how the rock is made visible, so what it should be is a
## property of what is being looked at.
func set_lamp(distance: float, angle_degrees: float, energy: float) -> void:
	_helmet_lamp.spot_range = distance
	_helmet_lamp.spot_angle = angle_degrees
	_helmet_lamp.light_energy = energy


## What the lamp does about shadow acne. Separate from set_lamp because these two
## answer the renderer rather than the lamp - see the Shadow artefacts region of
## ../drill_knobs.gd.
func set_lamp_shadows(reverse_cull: bool, normal_bias: float) -> void:
	_helmet_lamp.shadow_reverse_cull_face = reverse_cull
	_helmet_lamp.shadow_normal_bias = normal_bias


## Where the drill beam starts, and what it is aimed along.
...
func respawn() -> void:
	...
	_current_speed_cap = DrillMovementKnobs.MAX_SPEED
...
func _update_orientation(delta: float) -> void:
	...
	var is_inertial := DrillMovementKnobs.ROTATION_MODE == DrillMovementKnobs.RotationMode.INERTIAL
	if is_inertial:
		var aim_gain := (
			DrillMovementKnobs.MOUSE_SENSITIVITY * DrillMovementKnobs.ANGULAR_ACCELERATION
		)
		angular_velocity.x += -mouse_motion.y * aim_gain
		angular_velocity.y += -mouse_motion.x * aim_gain
		angular_velocity.z += -roll_input * DrillMovementKnobs.ROLL_RATE * delta
		angular_velocity = angular_velocity.limit_length(DrillMovementKnobs.MAX_ANGULAR_SPEED)
	...
	var aim_roll := 0.0
	if not is_inertial:
		aim_pitch = -mouse_motion.y * DrillMovementKnobs.MOUSE_SENSITIVITY
		aim_yaw = -mouse_motion.x * DrillMovementKnobs.MOUSE_SENSITIVITY
		aim_roll = -roll_input * DrillMovementKnobs.ROLL_RATE * delta
...
func _damp_angular_velocity(delta: float) -> void:
	...
	angular_velocity = angular_velocity.lerp(
		Vector3.ZERO, minf(DrillMovementKnobs.ANGULAR_DRAG * delta, 1.0)
	)
	if stabilizers_engaged:
		angular_velocity = angular_velocity.lerp(
			Vector3.ZERO, minf(DrillMovementKnobs.ANGULAR_STABILIZER_RATE * delta, 1.0)
		)
...
func _update_velocity(delta: float) -> void:
	...
	var thrust_acceleration := DrillMovementKnobs.THRUST_ACCELERATION
	if sprint_engaged:
		thrust_acceleration *= DrillMovementKnobs.SPRINT_ACCELERATION_MULTIPLIER

	velocity += global_transform.basis * thrust_input * thrust_acceleration * delta

	if stabilizers_engaged:
		velocity = velocity.lerp(
			Vector3.ZERO, minf(DrillMovementKnobs.LINEAR_STABILIZER_RATE * delta, 1.0)
		)
...
func _update_speed_cap(delta: float) -> void:
	var target_speed_cap := DrillMovementKnobs.MAX_SPEED
	if sprint_engaged:
		target_speed_cap *= DrillMovementKnobs.SPRINT_SPEED_MULTIPLIER

	if target_speed_cap >= _current_speed_cap:
		_current_speed_cap = target_speed_cap
	else:
		_current_speed_cap = move_toward(
			_current_speed_cap, target_speed_cap, DrillMovementKnobs.SPRINT_FALLOFF_RATE * delta
		)
...
func _resolve_hull_contact(
	...
	if was_already_touching:
		velocity = velocity.lerp(
			Vector3.ZERO, minf(DrillMovementKnobs.SCRAPE_FRICTION * delta, 1.0)
		)
		return
	...
	velocity = (
		along_surface * (1.0 - DrillMovementKnobs.COLLISION_FRICTION)
		- into_surface * DrillMovementKnobs.COLLISION_RESTITUTION
	)
...
func _shove_debris(
	...
	var restitution := 0.0 if was_already_touching else DrillMovementKnobs.COLLISION_RESTITUTION
	var reduced_mass := 1.0 / (1.0 / DrillMovementKnobs.PLAYER_MASS + 1.0 / debris.mass)
	var impulse_magnitude := -(1.0 + restitution) * closing_speed * reduced_mass

	velocity += contact_normal * (impulse_magnitude / DrillMovementKnobs.PLAYER_MASS)
	debris.apply_impulse(
		-contact_normal * impulse_magnitude, contact_point - debris.global_position
	)

	var mass_ratio := debris.mass / (debris.mass + DrillMovementKnobs.PLAYER_MASS)
...
func _apply_impact_spin(along_surface: Vector3, contact_point: Vector3) -> void:
	if is_zero_approx(DrillMovementKnobs.COLLISION_SPIN_TRANSFER):
		return

	var friction_impulse := -along_surface * DrillMovementKnobs.COLLISION_FRICTION
	...
	angular_velocity += (
		global_transform.basis.inverse() * world_torque * DrillMovementKnobs.COLLISION_SPIN_TRANSFER
	)
	angular_velocity = angular_velocity.limit_length(DrillMovementKnobs.MAX_ANGULAR_SPEED)

...
const VISOR_MATERIAL := preload("res://prefabs/character/miner/miner_visor_material.tres")
const GESTURE_LIBRARY := preload(
	"res://levels/asteroid_level/intro/animations/miner_gesture_library.tres"
)
...
## Emission on the visor before and after the suit comes online.
##
## AN ORDER OF MAGNITUDE BELOW A GREYBOX'S. The real character has one material
## for the whole helmet - dome, visor and all - so a number tuned against a small
## emissive visor box turns the head into a white blob that blows out the shot it
## is the climax of. Lit, not detonated.
...
@export var visor_energy: float = VISOR_IDLE_ENERGY:
	set(value):
		visor_energy = value
...
func set_visor_lit(lit: bool) -> void:
	visor_energy = VISOR_LIT_ENERGY if lit else VISOR_IDLE_ENERGY

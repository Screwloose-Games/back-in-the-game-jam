...
## Live-tunable, pushed by whichever cutscene owns this driver. Defaulted so the
## driver works standing alone, in a scene with no knobs file at all.
var rumble_amplitude := 0.012
var rumble_frequency := 17.0
var cutscene_fov := 62.0
...
func _anchor_pose() -> Transform3D:
	...
	if amplitude > 0.0:
		...
		var shake := Vector3(
			sin(_rumble_clock * rate), sin(_rumble_clock * rate * RUMBLE_AXIS_RATIO + 1.7), 0.0
		)

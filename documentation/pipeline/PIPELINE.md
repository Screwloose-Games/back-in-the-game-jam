...
- `assets/art/{category}/{object}/`
  - Everything belonging to one object: the mesh, its buffer, its textures, every .import sidecar, and the model container scene - and, for an object that also has 2D art, its .aseprite sources. One directory per object, so a re-export touches nothing else.
  - e.g. `assets/art/structures/raised_bed/`
- `prefabs/{category}/`
  - Prefab scenes, grouped by the same category names used under assets/art/.
...
4. Set the output directory to the asset's directory under assets/art/{category}/{object}/.
...
3. Export into assets/art/{category}/{object}/ using the sm_ or sk_ prefixed name.
...
Copy the exported .gltf, .bin and textures into assets/art/{category}/{object}/, overwriting what is there.
...
![GitHub Desktop on the solar-punk-jam repository showing five staged files under the raised_bed asset directory - the .png, .bin and .gltf plus both .import sidecars - committing to the asset-updates branch.](images/update_commit_with_github_desktop.png)
...
![Godot's FileSystem context menu on sm_rain_barrel.gltf in the structures category, with "New Inherited Scene" highlighted.](images/integration_new_inherited_scene.png)
...
- `assets/art/{category}/{object}/`
  - One directory per object, shared with that object's 3D art rather than split by media type. The .aseprite stem must match its parent directory, so assets/art/characters/spider/spider.aseprite is the only accepted form.
  - e.g. `assets/art/characters/spider/spider.aseprite`
